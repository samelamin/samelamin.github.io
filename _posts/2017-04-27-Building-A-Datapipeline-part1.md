...
# Building A Scalable And Reliable Dataµ Pipeline. Part 1
This post was inspired by a call I had with some of the Spark community user group on testing. If you haven't watch it then you will be happy to know that it was recorded, you can watch it [here](https://www.youtube.com/watch?v=2q0uAldCQ8M), there are some amazing ideas and thoughts being shared.
...
This is why I am hoping to build a series of posts explaining how I am currently building data pipelines, the series aims to construct a data pipeline from scratch all the way to a productionalised pipeline. Including a workflow manager and a dataserving layer.
...
## Tests Tests Tests!
As a strong advocate of test first development, I am starting the series on what inspired the original hangout call which is testing!
...
While the task itself isnt difficult, there are various scenarios that can make it so, e.g.:
 ...
 To tackle these issues what we do is the very first step in our pipeline is to clean the data and standardize it so it is easier for us to manage. For example dealing with GBs of data in RAW CSV and JSON is extremely difficult so we need to transform it to a format that is more manageable like [PARQUET](https://parquet.apache.org/) or [AVRO](https://avro.apache.org/docs/1.2.0/). We currently use PARQUET
